...
void VoxelFilter(std::vector<types::Point3D>& point_cloud, const double& resolution, const double& range_max)
{
    int index = std::numeric_limits<int>::max();
    std::map<int, std::vector<types::Point3D>> points_map;
    int grid_size = int(range_max / resolution);
    for (types::Point3D point : point_cloud)
    {
    ...
    {
        types::Point3D sum_point = types::Point3D::Zero();
        for (types::Point3D &p : point_pair.second)
            sum_point += p;
