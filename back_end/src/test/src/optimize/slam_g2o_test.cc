...
#include "optimize/factor_graph/types/bivariate_factor.h"
#include "optimize/factor_graph/core/graph_optimize.h"
#include "optimize/factor_graph/core/utils.h"
    ...
    bool LoadG2O(const std::string &filename, FactorGraph *graph)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            else if (data_type == "EDGE_SE3:QUAT")
            {
                ...
                ss >> id_a >> id_b >> point(0) >> point(1) >> point(2) >> q_x >> q_y >> q_z >> q_w >> i11 >> i12 >> i13 >> i14 >> i15 >> i16 >> i22 >> i23 >> i24 >> i25 >> i26 >> i33 >> i34 >> i35 >> i36 >> i44 >> i45 >> i46 >> i55 >> i56 >> i66;
                ...
                graph->AddFactor(new RotVec2RotVecFactor(id_to_pose[id_a], id_to_pose[id_b], measurement, info_mtrx));
            }
            else
            {
...
JUST_RUN_TEST(slam_g2o, test) 
TEST(slam_g2o, test) 
{
    std::string data_path = "../../src/test/data/torus3d_guess.g2o";
